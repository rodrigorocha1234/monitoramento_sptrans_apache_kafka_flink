...
class EstrategiaAvro(EstrategiaSerializacao[AvroSerializer]):

    def __init__(self):
        super().__init__()

        self.__schema_registry_client = SchemaRegistryClient({"url": (f"http://{Config.URL_SCHEMA_REGISTRY}:"
    ...
    def inicializar_serializacao(self) -> AvroSerializer:
        ...
        return AvroSerializer(self.__schema_registry_client, schema_str)
